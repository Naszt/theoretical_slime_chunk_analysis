...
std::vector<std::pair<int,i4>>join(i4 c){
  std::vector<std::pair<int,i4>>vec;i4 cnt=0;
...
i4 solve(i4 c1,i4 c2,int z,i4 cnt){
...
int main(){
  ...
  for(i4 i=0,coll=0,ans=0;;i++){
    printf("c:%u,元素:%u,碰撞:%u,解:%u\r",i,pos.size(),coll,ans);
    i4 c1=i;
    auto all_z=join(c1);
