...
void checkB(const i8 B){
  i8 gZ=g(Z),g0[Del],g1[Del];
  for(i8 i=1;i<Del;i++)g0[i]=g(Z+i),g1[i]=g(Z-i);
  #pragma omp parallel
  {
    std::vector<Node>local_posB;
    #pragma omp for nowait
      for(i8 c=B<<33;c<((B+1)<<33);c++){
        ...
        local_posB.push_back({c,up,down});
      }
    #pragma omp critical
      posB.insert(posB.end(),local_posB.begin(),local_posB.end());
  }std::sort(posB.begin(),posB.end(),[](const Node&a,const Node&b){return a.c<b.c;});
}
...
int main(){
  ...
  for(i8 B=0,ans=0;;++B){
    ...
    printf("\r〔找长条〕进度:%lld,个数:%lld\033[K\n",B+1,posB.size());
